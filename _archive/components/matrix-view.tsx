...
function RoutinePanel({
  ...
  const todayItems = useMemo(() => {
    const now = new Date();
    return items.filter((t) => expandRepeat(t.repeat, now, now).length > 0);
  }, [items]);
  ...
  return (
    <section
      style={{
        ...
        background: allDone ? "rgba(201,168,76,0.08)" : "rgba(255,255,255,0.04)",

        border: allDone ? "1px solid rgba(201,168,76,0.25)" : "1px solid rgba(255,255,255,0.08)",
    ...
    >
      ...
      >
        ...
        >
          {allDone ? <Sparkles size={12} /> : <CheckCircle2 size={12} />}

          <span>{assignee === "전체" ? "오늘 루틴" : `${assignee} 루틴`}</span>
        </div>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  background: doneToday ? "rgba(201,168,76,0.15)" : "rgba(255,255,255,0.04)",

                  color: doneToday ? "#f1d382" : "rgba(255,255,255,0.72)",
...
function MatrixSection({ title, subtitle, items, tone, onEdit, onToggle, onDelete }: any) {
  const toneStyles = {
    red: {
  ...
  const toneStyle = toneStyles[(tone as keyof typeof toneStyles) ?? "gray"];
...
export function MatrixView({ tasks, routines, showDone, onEdit, onToggle, onDelete }: Props) {
  const [activeTab, setActiveTab] = useState<AssigneeTab>("전체");

  const tabs: AssigneeTab[] = ["전체", "하나", "민효", "함께", "데이트"];

  const visibleTasks = useMemo(() => {
    const filtered = tasks.filter((t) => showDone || !t.done);

    return activeTab === "전체" ? filtered : filtered.filter((t) => t.assignee === activeTab);
  }, [tasks, showDone, activeTab]);

  const visibleRoutines = useMemo(
    () => (activeTab === "전체" ? routines : routines.filter((r) => r.assignee === activeTab)),
  ...
  return (
    ...
    >
          ...
          {tabs.map((tab) => (
            <button
              ...
              style={{
                ...
                color: activeTab === tab ? "var(--gold-soft)" : "rgba(255,255,255,0.65)",
      ...
      <RoutinePanel assignee={activeTab} items={visibleRoutines} onToggle={onToggle} />
