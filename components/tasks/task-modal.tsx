...
const ASSIGNEES: Task['assignee'][] = ['?òÎÇò', 'ÎØºÌö®', '?®Íªò', '?∞Ïù¥??]
const TYPES: { value: Task['type']; label: string }[] = [
  { value: 'ui',   label: 'Ï§ëÏöî¬∑Í∏¥Í∏â' },
  { value: 'nui',  label: 'Ï§ëÏöî' },
  { value: 'uni',  label: 'Í∏¥Í∏â' },
  { value: 'nuni', label: '?ºÎ∞ò' },
]
...
function normalizeTask(task?: Partial<Task> | null): Partial<Task> & { _hasTime: boolean } {
  ...
  return {
    id:         task?.id         || '',
    text:       task?.text       || '',
    assignee:   (task?.assignee as Task['assignee']) || '?òÎÇò',
    type:       task?.type       || 'ui',
...
export function TaskModal({ task, isOpen, onClose, onSave, isEditMode = false }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!form.text?.trim()) { alert('?ºÏ†ï ?úÎ™©???ÖÎ†•?¥Ï£º?∏Ïöî.'); return }
        ...
        <div style={{ padding: '16px 16px 12px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontSize: 16, fontWeight: 800, color: '#fff' }}>
              {isEditMode ? '?ºÏ†ï ?òÏ†ï' : '???ºÏ†ï Ï∂îÍ?'}
            </div>
            <div style={{ marginTop: 4, fontSize: 11, color: 'rgba(255,255,255,0.45)' }}>
              ?ÑÏöî???ïÎ≥¥Îß?Í∞ÄÎ≥çÍ≤å ?ÖÎ†•?¥ÎèÑ ?ºÏöî
            </div>
          </div>
          <button type="button" onClick={onClose} style={iconBtnStyle} title="?´Í∏∞">
            <X size={16} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ maxHeight: '70vh', overflowY: 'auto', padding: '14px 16px 16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <FieldLabel>?ºÏ†ï ?úÎ™©</FieldLabel>
              <textarea
                value={form.text || ''}
                onChange={(e) => updateField('text', e.target.value)}
                rows={3}
                placeholder="?? Î≥ëÏõê ?àÏïΩ, Í∏∞Ìöç??ÎßàÍ∞ê, Í∞ÄÏ°??∏Ïãù"
                style={textareaStyle}
              />
            </div>
            ...
            {/* ?¥Îãπ??+ ?∞ÏÑ†?úÏúÑ */}
            <div style={grid2}>
              <FieldBlock icon={<UserRound size={13} />} label="?¥Îãπ??>
                <select value={form.assignee || '?òÎÇò'} onChange={(e) => updateField('assignee', e.target.value)} style={selectStyle}>
                  {ASSIGNEES.map((name) => (
                    <option key={name} value={name} style={{ color: '#111' }}>{name}</option>
                  ))}
                </select>
              </FieldBlock>
              <FieldBlock icon={<LayoutGrid size={13} />} label="?∞ÏÑ†?úÏúÑ">
            ...
            <div style={{ padding: '10px 12px', borderRadius: 14, background: typePreview.bg, border: '1px solid rgba(255,255,255,0.06)', color: typePreview.color, fontSize: 12, fontWeight: 700 }}>
              ?ÑÏû¨ ?†ÌÉù: {TYPES.find((t) => t.value === form.type)?.label || '?ºÎ∞ò'}
            </div>

            {/* ?úÏûë??+ Ï¢ÖÎ£å??*/}
            <div style={grid2}>
              <FieldBlock icon={<CalendarDays size={13} />} label="?úÏûë??>
            ...
            {/* ?•ÏÜå */}
            <FieldBlock icon={<MapPin size={13} />} label="?•ÏÜå">
              <input type="text" value={form.location || ''} onChange={(e) => updateField('location', e.target.value)} placeholder="?? ?°ÎèÑ / Î≥ëÏõê / Ïß? style={inputStyle} />
            </FieldBlock>

            {/* Î∞òÎ≥µ ?îÏùº Î©Ä???†ÌÉù */}
            <FieldBlock icon={<Repeat size={13} />} label="Î∞òÎ≥µ ?îÏùº (Î£®Ìã¥)">
              ...
              {repeatDays.length > 0 && (
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', marginTop: 4 }}>
                  Îß§Ï£º {repeatDays.map((d) => DAY_LABELS[d]).join('¬∑')} Î∞òÎ≥µ
                </div>
              )}
            </FieldBlock>
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 12px', borderRadius: 16, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', cursor: 'pointer' }}>
              ...
              />
              <div>
                <div style={{ fontSize: 12, fontWeight: 700, color: '#fff' }}>?ÑÎ£å ?ÅÌÉúÎ°??Ä??/div>
                <div style={{ marginTop: 2, fontSize: 10, color: 'rgba(255,255,255,0.42)' }}>?¥Î? ?ùÎÇú ?ºÏ†ï?¥Î©¥ Ï≤¥ÌÅ¨?¥ÎëêÎ©??ºÏöî</div>
              </div>
            </label>
          </div>
          ...
          <div style={{ padding: '12px 16px 16px', borderTop: '1px solid rgba(255,255,255,0.06)', display: 'flex', gap: 8 }}>
            <button type="button" onClick={onClose} style={secondaryBtnStyle}>Ï∑®ÏÜå</button>
            <button type="submit" disabled={isSubmitting} style={{ ...primaryBtnStyle, opacity: isSubmitting ? 0.6 : 1, cursor: isSubmitting ? 'default' : 'pointer' }}>
              {isSubmitting ? '?Ä??Ï§?..' : isEditMode ? '?òÏ†ï ?ÑÎ£å' : '?ºÏ†ï ?Ä??}
            </button>
          </div>
        </form>
      </div>
