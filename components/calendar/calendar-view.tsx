...
function formatSelectedDate(dateStr: string) {
  ...
  const week = ["??, "??, "??, "??, "ëª?, "ê¸?, "??];
  return `${year}??${month}??${day}??${week[d.getDay()]}?”ì¼`;
}

function getTaskTypeLabel(task: Task) {
  if (task.repeat && task.repeat !== "none") return "ë£¨í‹´";
  if (task.type === "meeting") return "ë¯¸íŒ…";
  if (task.type === "work") return "?…ë¬´";
  if (task.type === "life") return "?í™œ";
  if (task.type === "ui") return "?¼ì •";
  return "?¼ì •";
}
      ...
      <div className="calendar-shell">
        <div className="calendar-topbar">
          <div>
            ...
            <h2 className="calendar-topbar__title">?œëˆˆ??ë³´ëŠ” ?¼ì •</h2>
          </div>
